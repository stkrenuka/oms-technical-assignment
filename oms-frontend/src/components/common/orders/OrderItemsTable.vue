  ...
  <table class="w-full text-sm mb-2">
    ...
    <tbody>
      <tr v-for="(item, i) in items" :key="i">
        <td class="border p-2">
          <ProductSearch @select="product => emit('product-selected', { product, item })" />
        </td>
        <td class="border p-2">
          <input v-model.number="item.qty" type="number" min="1" class="border px-2 py-1 rounded w-20" />
        </td>
        <td class="border p-2">
          <input type="number" :value="item.price" class="border px-2 py-1 rounded w-24 bg-gray-100" readonly />
        </td>
