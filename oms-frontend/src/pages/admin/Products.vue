  ...
  <DefaultLayout>
    <div class="bg-white rounded shadow p-6">
      ...
      <div class="flex justify-between mt-4">
        <button class="px-4 py-2 bg-gray-300 rounded" :disabled="!productStore.pagination.prev_page"
        ...
        <button class="px-4 py-2 bg-gray-300 rounded" :disabled="!productStore.pagination.next_page"
          @click="productStore.nextPage">
          Next
        </button>
      </div>
      <!-- Modal -->
      <div v-if="productStore.showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center">
        <div class="bg-white p-6 rounded w-96">
          <h3 class="text-lg font-semibold mb-4">
            {{ productStore.isEdit ? 'Edit Product' : 'Add Product' }}
          </h3>
          <input v-model="productStore.form.name" placeholder="Product name" class="w-full mb-2 border p-2 rounded" />
          <p v-if="productStore.errors.name" class="text-red-500 text-sm mt-1">
            {{ productStore.errors.name[0] }}
          </p>
          <input v-model="productStore.form.price" type="number" placeholder="Price"
          ...
          <input v-model="productStore.form.stock" type="number" placeholder="Stock"
            class="w-full mb-2 border p-2 rounded" />
          ...
          <div class="flex justify-end space-x-2">
            ...
            <button class="px-4 py-2 bg-green-600 text-white rounded" @click="productStore.saveProduct">
              Save
            </button>
          </div>
        </div>
